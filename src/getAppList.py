...
def getAppDetails(app_url):
    ...
    price_buy_span = soup.find('span', {'class': 'display-price'})
    # price = price_buy_span.find_all('span')[-1].get_text().strip()
    price = price_buy_span.text
    # price = price[:-4].strip() if price != 'Install' else 'Free'
    # price = price_buy_span.text if price != 'Install' else 'Free'
    if price == 'Free':
       applist.write(app_url[23:] + '\n')
